...
| D022 | M001 | scope | Auth in MVP | Anonymous only — Apple/email buttons visible for layout fidelity; all three Welcome buttons route to anonymous-start in v1 (no stub toast) | Ship faster; v1 is local-first so no sync to gate. Supabase Auth in v2 | Yes — pull forward if demand |
...
| D029 | M001 | pattern | Typography — user-selectable alternatives | 4 display + 6 reader + 2 UI + 2 mono options | Design handoff tweaks panel exposes these | **Superseded by D030** |
| D030 | M001 | scope | Typography — user-facing picker | Reader-font only (5 options: EB Garamond, Fraunces, Georgia, Inter, JetBrains Mono). Display/UI/mono stay fixed in v1 (supersedes D029) | Handoff tweaks panel was a *designer* knob, not a user-facing setting. Settings screen shows only "Default font" → keep scope tight | Yes — expose others in v2 if demand |
| D031 | M001 | scope | Testing in MVP | Vitest unit tests only; no Playwright E2E, no integration harness | E2E adds toolchain weight that MVP doesn't need; regression coverage lands in v2 if it becomes a problem | Yes |
| D032 | M001 | scope | Settings screen — v1 subset | No ACCOUNT section; ABOUT shows Version only; no Privacy/Terms/Acknowledgements links until copy exists | Non-functional rows create trust drag; add when real | Yes |
| D033 | M001 | scope | Accessibility — screen readers | Mark reader stage `aria-hidden`, surface a sibling explaining RSVP/SR incompatibility. No in-app SpeechSynthesisUtterance fallback in v1 | Fallback mode is ~100 lines for a marginal audience; honest exclusion is better than broken promise | Yes — v2 if demand |
| D034 | M001 | scope | Performance targets — enforceable only | Keep: word-timing ±20ms, word-render <50ms, bundle <180KB gzip, Lighthouse PWA ≥90, Perf ≥85. Drop: 60fps scroll, <500ms repeat-visit, <1.8s LCP, memory footprint | Aspirational targets without measurement infrastructure are noise; measure what we can gate on | Yes |
